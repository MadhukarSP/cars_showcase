...
const CustomFilter = ({ title, options }: CustomFilterProps) => {
    ...
    const handleUpdateParams = (e: { title: string, value: string }) => {
        const newURL = updateSearchParams(title, e.value.toLowerCase());
                        ...
                        <Listbox.Options className="custom-filter__options">
                            {options.map((option: OptionProps) => (
                                <Listbox.Option key={option.title} value={option}
                                    className={({ active }) => `relative cursor-default, select-none py-2 px-4 ${active ? 'bg-primary-blue text-white' : 'text-grey-100'}`}
                                >
